...
func (s *obligationService) listTenantBillingHistoryPage(ctx context.Context, userID, tenantID uint64, fromMonth, toMonth time.Time, page, pageSize int) (tenantBillingHistoryPage, error) {
	...
	history := buildTenantBillingHistoryWithCharges([]tenant{tenantRow}, obligations, paymentRows, time.Now().UTC(), charges)[tenantID]
	currentPeriod := dublinCurrentMonth(time.Now()).Format("2006-01")
	currentMonth, hasCurrentMonth := aggregateTenantBillingMonth(history, currentPeriod)
...
func (s *obligationService) listTenantPaidByOtherRows(ctx context.Context, userID, tenantID uint64, fromMonth, toMonth time.Time) ([]tenantPaidByOtherRow, error) {
	...
	for _, item := range raw {
		...
		rows = append(rows, tenantPaidByOtherRow{
			...
			OwnerName:   firstNonEmpty(nameByID[item.OwnerID], "未知租客"),
...
<head>
  ...
  <style>`+workspacePageCSS+`
    ...
    @media (max-width: 640px) {
      ...
      .tenant-detail-page { padding-bottom: 116px; }
      ...
      .tenant-detail-mobile-actions .delete-object-form { display: contents; }
...
<header class="tenant-detail-head"><a class="tenant-detail-back" href="/tenants{{if .ReturnSearch}}?search={{urlquery .ReturnSearch}}{{if .ReturnSort}}&amp;sort={{urlquery .ReturnSort}}{{end}}{{else if .ReturnSort}}?sort={{urlquery .ReturnSort}}{{end}}">← 返回租客管理</a><div class="tenant-identity-row"><div class="tenant-identity"><div class="identity-mark">TN</div><div><div class="brand-title">租客详情</div><h1>{{if .Tenant.DisplayAlias}}{{.Tenant.DisplayAlias}}{{else}}{{.Tenant.Name}}{{end}}</h1><div class="tenant-identity-meta"><span>姓名：{{.Tenant.Name}}</span>{{if .Tenant.Email}}<span>邮箱：{{.Tenant.Email}}</span>{{end}}<span class="detail-status {{.Tenant.Status}}">{{if eq .Tenant.Status "active"}}有效档案{{else}}已停用{{end}}</span></div></div></div><div class="actions"><a class="btn" href="{{.EditURL}}">编辑资料</a><a class="btn primary" href="{{.CashReceiptOpenURL}}">录入现金收款</a><form method="post" action="/tenants/{{.Tenant.ID}}" class="delete-object-form"><input type="hidden" name="action" value="delete"><input type="hidden" name="search" value="{{.ReturnSearch}}"><input type="hidden" name="sort" value="{{.ReturnSort}}"><button class="btn danger" type="submit" data-confirm="true" data-confirm-title="删除租客" data-confirm-label="确认删除租客" data-confirm-message="删除租客「{{.Tenant.Name}}」及其档案？若仍有关联房间或租金记录，系统会拒绝。">删除租客</button></form></div></div></header>
	...
	</section><section class="panel surface tenant-paid-for-panel" aria-labelledby="paid-for-title"><div class="panel-head"><div><h2 id="paid-for-title">代付与被代付</h2><p class="tiny">代付只影响收款记录，每位租客的应收租金保持独立。</p></div></div>{{if .PaidByOtherRows}}<div class="table-wrap"><table class="paid-for-table"><thead><tr><th>租金月份</th><th>实际付款人</th><th>入账租客</th><th class="amount">分配金额</th><th>结果</th></tr></thead><tbody>{{range .PaidByOtherRows}}<tr><td class="mono">{{.PeriodLabel}}</td><td>{{.PayerName}}{{if eq .Result "本人被代付"}} <span class="payer-badge">代付</span>{{end}}</td><td>{{.OwnerName}}</td><td class="amount">{{.Amount}}</td><td><span class="status paid">{{.Result}}</span></td></tr>{{end}}</tbody></table></div>{{else}}<div class="empty">所选期间没有代付或代收记录。</div>{{end}}</section><section class="panel surface tenant-responsibility-panel" aria-labelledby="responsibility-title"><div class="panel-head"><h2 id="responsibility-title">租金与代付</h2><span class="tiny">{{.CurrentPeriod}}</span></div><div class="tenant-responsibility-list"><div class="tenant-responsibility-row"><span><strong>本月应收租金</strong><small>由房间入住与租金计划生成</small></span><strong>{{if .HasCurrentBilling}}{{.CurrentBilling.ExpectedAmount}}{{else}}—{{end}}</strong></div><div class="tenant-responsibility-row"><span><strong>已确认收款</strong><small>{{if .HasCurrentBilling}}{{if .CurrentBilling.Payments}}{{range .CurrentBilling.Payments}}{{.Source}} {{.AmountDisplay}} · {{.DateDisplay}} {{end}}{{else}}暂无有效收款{{end}}{{else}}当月暂无账单{{end}}</small></span><strong>{{if .HasCurrentBilling}}{{.CurrentBilling.PaidAmount}}{{else}}—{{end}}</strong></div></div></section><aside class="tenant-detail-side"><section class="panel surface" aria-labelledby="profile-title"><div class="panel-head"><h2 id="profile-title">租客档案</h2><span class="tiny">档案信息</span></div><dl class="profile-list"><dt>邮箱</dt><dd>{{if .Tenant.Email}}{{.Tenant.Email}}{{else}}未填写{{end}}</dd><dt>状态</dt><dd>{{if eq .Tenant.Status "active"}}有效档案{{else}}已停用{{end}}</dd><dt>档案建立</dt><dd>{{.Tenant.CreatedAt}}</dd></dl><p class="tiny">入住关系、租金和应收金额由对应房间的“入住与租金”计划维护。</p></section><section class="panel surface" aria-labelledby="payer-title"><div class="panel-head"><h2 id="payer-title">付款识别</h2><span class="tiny">仅保存用于匹配的付款人名称与稳定 ID</span></div><div class="payer-list">{{if .Payers}}{{range .Payers}}<div class="payer-item"><div class="payer-meta"><strong>{{.Name}}</strong><span class="mono">{{if .PayerID}}ID: {{.PayerID}}{{else}}仅名称，无稳定 ID{{end}}</span>{{if .Shared}}<span class="flag">共享／冲突候选，不能自动选租客</span>{{end}}{{if .RemovedAt}}<span class="tiny">已移除：{{.RemovedAt}}</span>{{end}}</div>{{if not .RemovedAt}}<form method="post" action="/tenants/{{$.Tenant.ID}}/payers/remove"><input type="hidden" name="payer_id" value="{{.ID}}"><button class="btn subtle" type="submit" data-confirm="true" data-confirm-title="移除付款人关系" data-confirm-label="确认移除" data-confirm-message="移除付款人「{{.Name}}」与租客「{{$.Tenant.Name}}」的识别关系？历史收款记录不会改变，之后的流水将不再使用这条关系自动识别。">移除</button></form>{{end}}</div>{{end}}{{else}}<div class="empty">暂无付款人关系。</div>{{end}}</div><form method="post" action="/tenants/{{.Tenant.ID}}/payers" class="form"><label for="payer_name">付款人名称</label><input id="payer_name" name="payer_name" placeholder="例如 Mike" required><label for="payer_id_detail">稳定付款人 ID（可选）</label><input id="payer_id_detail" name="payer_id" placeholder="仅在银行明确提供时填写"><small class="tiny">IE… 是单笔流水号，不要填入付款人 ID。</small><button class="btn primary" type="submit">添加付款人</button></form></section><div class="risk-note">是否欠租只看个人应收租金的未收金额。即使由他人代付，完整覆盖后也不会进入催收。</div></aside></div><div class="tenant-detail-mobile-actions"><a class="btn" href="{{.EditURL}}">编辑资料</a><a class="btn primary" href="{{.CashReceiptOpenURL}}">现金补录</a><form method="post" action="/tenants/{{.Tenant.ID}}" class="delete-object-form"><input type="hidden" name="action" value="delete"><input type="hidden" name="search" value="{{.ReturnSearch}}"><input type="hidden" name="sort" value="{{.ReturnSort}}"><button class="btn danger" type="submit" data-confirm="true" data-confirm-title="删除租客" data-confirm-label="确认删除租客" data-confirm-message="删除租客「{{.Tenant.Name}}」及其档案？若仍有关联房间或租金记录，系统会拒绝。">删除租客</button></form></div></div>
  </main>
</div></body></html>`)
...
func (a *app) handleTenantDetail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "tenant detail unavailable", http.StatusInternalServerError)
		return
	}
	...
	data := tenantDetailPageData{
		...
		HasCurrentBilling: history.HasCurrentMonth,
	}
	for _, item := range history.Rows {
		if item.Period == data.CurrentPeriod && item.Status != obligationRecordVoided {
...
func (a *app) handleTenantSubroute(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		a.handleTenantMutation(w, r)
		return
	}
	a.handleTenantDetail(w, r)
}

func (a *app) handleTenantMutation(w http.ResponseWriter, r *http.Request) {
	if !a.requireAuth(w, r) {
		return
	}
	userID, ok := a.currentUserID(r)
	...
	if err != nil || tenantID == 0 || strings.Contains(rawID, "/") {
		http.NotFound(w, r)
		return
	}
	if err := r.ParseForm(); err != nil {
		http.Redirect(w, r, tenantDeleteDetailURL(tenantID, "", "invalid_form"), http.StatusFound)
		return
	}
	if r.Form.Get("action") != "delete" {
		http.Error(w, "tenant action is invalid", http.StatusBadRequest)
		return
	}
	if err := newTenantService(a.db).deleteTenant(r.Context(), userID, tenantID); err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			http.NotFound(w, r)
			return
		}
		if errors.Is(err, errTenantDeletionBlocked) {
			http.Redirect(w, r, tenantDeleteDetailURL(tenantID, r.Form.Get("search"), "tenant_delete_blocked"), http.StatusFound)
			return
		}
		http.Error(w, "tenant deletion failed", http.StatusInternalServerError)
		return
	}
	query := url.Values{"message": []string{"tenant_deleted"}}
...
func tenantDeleteDetailURL(tenantID uint64, search, errorCode string) string {
	query := url.Values{}
	if search = strings.TrimSpace(search); search != "" {
		query.Set("search", search)
	}
	...
	return "/tenants/" + strconv.FormatUint(tenantID, 10) + "?" + query.Encode()
}
