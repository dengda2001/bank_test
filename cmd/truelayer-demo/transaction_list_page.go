...
<head>
  ...
  <style>`+workspacePageCSS+`
    ...
    @media (max-width: 640px) {
      ...
      .transaction-route-quickfilter { display: grid; grid-template-columns: repeat(2,minmax(0,1fr)); margin-bottom: 12px; }
      .transaction-route-quickfilter select { width: 100%; min-width: 0; min-height: 44px; }
      .transaction-route-quickfilter input[type="search"] { grid-column: 1/-1; min-width: 0; }
  ...
  <link rel="stylesheet" href="/static/css/pages/transaction-expense.css">
</head>
<body>
  <div class="app">
    ...
    <main class="content transaction-route-page">
      ...
      <form class="transaction-route-quickfilter" method="get" action="/transactions" aria-label="搜索流水">
        ...
        <select id="direction" name="direction" aria-label="收支方向" onchange="this.form.requestSubmit()"><option value="" {{if eq .DirectionFilter ""}}selected{{end}}>全部收支</option><option value="income" {{if eq .DirectionFilter "income"}}selected{{end}}>收入</option><option value="expense" {{if eq .DirectionFilter "expense"}}selected{{end}}>支出</option></select>
        ...
        <div class="transaction-route-mobile-list">
          ...
          <article class="transaction-review-card" id="mobile-transaction-{{.DetailKey}}" data-direction="{{.Direction}}">
            ...
            <div class="transaction-review-actions">
              ...
              {{if .MatchURL}}<a class="btn primary" href="{{.MatchURL}}" data-match-trigger="{{.InternalID}}">{{if or (eq .MatchStatus "matched") (eq .MatchStatus "partial")}}调整分配{{else}}处理分配{{end}}</a>{{end}}{{if .ExpenseLinkURL}}<a class="btn primary" href="{{.ExpenseLinkURL}}">{{if .ExpenseLinked}}编辑关联{{else}}关联房间{{end}}</a>{{end}}
              {{if and (eq .Direction "income") (or (eq .MatchStatus "matched") (eq .MatchStatus "partial"))}}<a class="btn subtle danger" href="/transactions/revoke?transaction_id={{.ID}}&amp;return_to={{.ReturnURL | urlquery}}">撤销整笔匹配</a>{{end}}
            </div>
          </article>
          {{end}}
        </div>
