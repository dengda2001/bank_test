...
body{margin:0;padding:32px;background:#050506;color:#ededef;font-family:system-ui,sans-serif}.card{max-width:760px;margin:auto;padding:24px;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:#111218}h1{margin-top:0}dt{color:#8a8f98;margin-top:14px}dd{margin:4px 0 0;overflow-wrap:anywhere}.allocation{padding:12px 0;border-top:1px solid rgba(255,255,255,.1)}label{display:block;margin-top:18px;color:#8a8f98}input{width:100%;box-sizing:border-box;padding:10px;border:1px solid rgba(255,255,255,.16);border-radius:8px;color:#ededef;background:#050506}button,a{display:inline-block;margin-top:16px;padding:10px 14px;border:0;border-radius:8px;color:#fff;background:#5e6ad2;text-decoration:none;cursor:pointer}.back{margin-left:8px;color:#ededef;background:rgba(255,255,255,.08)}.muted{color:#8a8f98;font-size:13px}
</style></head><body><main class="card"><h1>撤销整笔匹配</h1><p class="muted">请核对原流水及所有有效分配。提交后只会作废这笔来源的当前有效分配，不删除银行原文。</p><dl><dt>原流水</dt><dd>{{.Description}}</dd><dt>来源金额</dt><dd>{{.AmountDisplay}}</dd><dt>当前已分配</dt><dd>{{.AllocatedAmountDisplay}}</dd><dt>当前余款</dt><dd>{{.CurrentRemainingAmountDisplay}}</dd><dt>撤销后余款</dt><dd>{{.RemainingAmountDisplay}}</dd></dl><h2>有效分配</h2>{{range .Allocations}}<div class="allocation"><strong>{{.Kind}}</strong> · {{.AmountDisplay}}{{if .TenantName}} · 租客：{{.TenantName}}{{end}}{{if .PeriodDisplay}} · 租金月：{{.PeriodDisplay}}{{end}}{{if .Note}}<div class="muted">备注：{{.Note}}</div>{{end}}</div>{{end}}<form method="post" action="/billing/revoke"><input type="hidden" name="transaction_id" value="{{.TransactionID}}"><label for="reason">撤销原因（必填）</label><input id="reason" name="reason" required maxlength="512"><button type="submit">确认撤销</button><a class="back" href="/billing">返回流水</a></form></main></body></html>`))
...
func transactionRevokePreviewDataFromModel(preview transactionRevokePreview) transactionRevokePreviewData {
...
body{margin:0;padding:32px;background:#050506;color:#ededef;font-family:system-ui,sans-serif}.card{max-width:1100px;margin:auto;padding:24px;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:#111218}table{width:100%;border-collapse:collapse}th,td{padding:12px 10px;border-bottom:1px solid rgba(255,255,255,.1);text-align:left;vertical-align:top}th{color:#8a8f98;font-size:12px}input{padding:8px;border:1px solid rgba(255,255,255,.16);border-radius:8px;color:#ededef;background:#050506}button,a{display:inline-block;padding:8px 12px;border:0;border-radius:8px;color:#fff;background:#5e6ad2;text-decoration:none;cursor:pointer}.muted{color:#8a8f98;font-size:13px}.action{display:grid;gap:6px;min-width:180px}.back{margin-top:16px;color:#ededef;background:rgba(255,255,255,.08)}
</style></head><body><main class="card"><h1>历史待处理付款人预览</h1><p class="muted">以下结果是逐笔预览。共享付款人、缺少月份或有冲突的流水不会被批量自动确认；请逐笔核对后提交。</p>{{if .Rows}}<table><thead><tr><th>流水</th><th>付款人</th><th>金额</th><th>建议</th><th>状态／原因</th><th>操作</th></tr></thead><tbody>{{range .Rows}}<tr><td>#{{.TransactionID}}</td><td>{{.PayerName}}<br><span class="muted">{{.PayerID}}</span></td><td>{{.AmountDisplay}}</td><td>{{if .ProposedTenantName}}{{.ProposedTenantName}}{{else}}未确定{{end}}<br>{{if .ProposedPeriod}}{{.ProposedPeriod}}{{else}}月份待选{{end}}</td><td>{{.StatusLabel}}{{if .Reason}}<br><span class="muted">{{.Reason}}</span>{{end}}</td><td>{{if .CanConfirm}}<form class="action" method="post" action="/billing/payer/confirm"><input type="hidden" name="transaction_id" value="{{.TransactionID}}"><input type="hidden" name="tenant_id" value="{{.TenantID}}"><input name="period" value="{{.ProposedPeriod}}" placeholder="YYYY-MM" required><label class="muted"><input type="checkbox" name="remember_payer" value="1" checked> 记住付款人</label><button type="submit">确认此笔</button></form>{{else}}<span class="muted">需人工处理</span>{{end}}</td></tr>{{end}}</tbody></table>{{else}}<p class="muted">没有可预览的历史待处理流水。</p>{{end}}<a class="back" href="/billing">返回流水</a></main></body></html>`))
