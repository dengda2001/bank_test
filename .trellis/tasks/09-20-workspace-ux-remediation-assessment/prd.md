...
## Workspace polish follow-up added 2026-09-20

The user added seven follow-up fixes after the interaction implementation: remove room type/capacity from room creation, editing and details while preserving stored values; show the dashboard manual-review queue as green when empty; fix the tenant popup inside the processing dialog; auto-submit transaction match-status changes; add searchable tenant selectors; prevent property/room collection-status labels from truncating; and keep “查看详情” available on every transaction row. These requirements and acceptance criteria are tracked in child task [`09-20-workspace-ui-polish`](../09-20-workspace-ui-polish/prd.md).
