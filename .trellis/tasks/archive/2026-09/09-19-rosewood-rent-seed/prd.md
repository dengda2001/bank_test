...
- **撤回（2026-09-19）** —— 原「`/rent-dashboard` 切换 2026-05 ~ 2026-12 各月，应收合计不为 0
  且随月份变化」一条**不再作为本任务的验收项**：它写得有误，假定该页读义务表，
  实际该路由在有库时渲染房间视角，取数自带 `rent_charge_id IS NOT NULL`
  （`rent_workspace.go:1118`），而仓库里没有任何生产代码写 `rent_charges`。——
  这靠灌数据**无法满足**，是产品语义缺口而非数据缺口。根因与两个候选方案见
  `implement.md`「处置决定」，已转出到独立任务。其余 6 条全部通过。
...
- **`/rent-dashboard` 房间视角的应收金额（charge 路径）**。见验收项里那条「撤回」：
  该视角的金额只来自 `rent_charges`，而两条义务模型（惰性 / charge 背书）目前互斥
  （`obligations.go:139` 的 `MonthlyRentCents <= 0` 守卫），本任务按源表「应收」写的是惰性路。
  这是产品语义决策，不是灌数据能解决的，转出到独立任务。
