...
**对本子任务的影响（已解决）**：验收项「四档 11 页 `scrollWidth === clientWidth`」与
「四档截图存入 `research/screenshots/`」当时都依赖这个实例，故曾**挂起**。
**2026-09-19 已恢复并执行完毕**：B1–B3 修好后 seeder 全过，`desktop-widths.mjs` 跑出
`exit 0` / 44 项 `overflow=0`，44 张截图已提交。上面那段二选一的恢复条件因此**不再需要执行**
—— 记录保留，因为「演示位被标成 `ignored` 导致断言永远不通过」是个值得记住的失败模式。

在这之前，本子任务可做的部分是：第 1–4 步、第 6–7 步（spec 改写、1100 档落地、980/981 处置、断言同步）与第 9 步的 Go 测试部分。

### B3 审计的 dashboard 断言是结构性死代码（已修，见提交 `15d49bb`）
