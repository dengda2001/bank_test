...
- [x] 跨用户 ID 查询不到数据，越权写入被拒绝或返回安全的不存在结果。
- [x] 已提供有效租约、房间责任、付款分配和房产/房间支出的用户隔离读取边界。
- [x] 缺少关系、空列表和不存在房间有稳定结果，不回退到文本匹配。
- [x] Repository/Query 层已覆盖所有权、月份过滤和关键关联；真实 MySQL 集成因本地未配置 DSN 按约定跳过。
...
验收记录（2026-09-18）：用户隔离、缺失用户和静态编译回归通过，`go test ./cmd/truelayer-demo -count=1`、`go test ./...` 和 `go vet ./...` 通过。两组 Repository MySQL 集成测试因 `RENTOPS_MYSQL_TEST_DSN` 未设置按用户约定跳过。
